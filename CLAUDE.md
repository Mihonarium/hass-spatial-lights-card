...
**Modes:**
- Locked (default `_lockPositions = true`): tap → select / toggle (per `switch_single_tap` / `light_single_tap`, via `_togglesOnSingleTap`; modifier-click on a light still selects); 500 ms long-press → more-info. The canvas gets `touch-action: pan-y pinch-zoom` (class `touch-scroll`, gated by `canvas_touch_scroll`) so vertical swipes scroll the page.
