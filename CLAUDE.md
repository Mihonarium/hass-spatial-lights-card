...
- Rubber-band: a pointerdown on empty canvas arms `_selectionStart`/`_selectionPointerId`; the `.selection-box` materializes only after 5 px of movement (so browser scroll takeovers keep the selection), hit-testing is rAF-coalesced and diffed, and a completed tap (not pointerdown) is what deselects. On touch, a ~300 ms still hold (`_selectionHoldTimer` → `_selectionTouchArmed`) locks the gesture for an any-direction marquee: a non-passive canvas `touchmove` listener preventDefaults while armed so the browser can't reclaim the drag as a scroll, and `_handleCanvasContextMenu` swallows Android's ~500 ms long-press contextmenu while armed.
