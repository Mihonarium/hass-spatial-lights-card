...
**Undo/redo:** every card-initiated write calls `_captureUndo(kind)` first, which snapshots `_undoEntityPool()` (config entities + `default_entity` + group members via the `entity_id` attribute, recursive) into a step on `_lightUndoStack` (20 steps; both stacks clear after 30 min idle; same-kind calls within 1 s coalesce so a live wheel drag is one step; a new step clears `_lightRedoStack`). `_settleUndoStep` runs 2.5 s after the step's last call and records `changed` + `after` (entities whose `_snapshotEntityState` differs from `before`). `_applyHistoryStep(direction)` restores `before` (undo) or `after` (redo) for `changed` only, skipping entities whose current state no longer matches what the step left (someone else changed them), restoring groups through their members, batching identical payloads, and moving the step to the other stack (back on failure). `#undoBtn`/`#redoBtn` live in `.history-group` at the right end of `.presets-row` (44 px hit area, disabled = 38 % opacity, one pulse per session on first enable, one-time per-device hint via localStorage `UNDO_HINT_KEY`); `_updateHistoryButtons` syncs them. Ctrl/Cmd+Z and Ctrl+Y/Cmd+Shift+Z prefer light undo/redo outside edit mode. Config: `undo` (default true).
