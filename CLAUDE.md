...
- Group entities (`highlight_group_members`, default on): the selected group stays the operand; `_displaySelection()` adds its on-canvas members (via `_expandGroupMembers` → HA `entity_id` attribute or `_zigbeeGroups`) as `.selected.via-group` and marks the group `.group-selected` in `updateLights()`; `_selectionAfterTap(entity, additive)` is the single tap/Enter rule (a modifier-tap on a via-group member replaces the group by its on-canvas members minus that one); `_getControlledEntities()` drops explicitly selected members covered by a selected group and is used by the wheel/preset/slider write paths.
