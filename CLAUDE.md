...
**Undo/redo:** one history of two step kinds. Card steps: every write first calls `_captureUndo(kind, targets, {live})`, which snapshots `_expandUndoTargets(targets)` (targets + group members via the `entity_id` attribute or `_zigbeeGroups`; null → all card entities + `default_entity`) as `before` and marks them in `_expectedChanges` (`{until: +15 s, want}`; `_svc`/restores also record the asked-for end state per target so a late report that matches it is still ours, see `_isOwnChange`/`_matchesWant`); writes go through `this._svc(...)` so calls are recorded for redo. Discrete actions are always new steps; only a live wheel drag (`live`) or repeated brightness/temperature commits on the same `targetsKey` within 1 s extend the open step. External steps (`undo_external`): `_trackExternalChanges(prev, next)` runs from `set hass`, records changes of leaf pool entities (never groups or the `default_entity`, see `_isDerivedEntity`) that are not expected and exceed the loose thresholds of `_sameEntityState(…, true)` (drift such as adaptive lighting is ignored), grouping arrivals within 2 s; external steps do not clear redo. `_undoLightState` restores `before` unconditionally via `_restoreCalls` → `_restorePayload` (native colour attribute of the recorded `color_mode`, HA-scene style; groups through members; identical payloads batched and then covered with Z2M groups via `_planGroupedDispatch` so a same-payload set is one groupcast; entities already equal are skipped only once the step is >10 s old). `_redoLightState` replays a card step's calls (last per target set) or re-applies an external step's `after`. Failed calls move the step back. Buttons: `#undoBtn`/`#redoBtn` in `.history-group` at the right end of `.presets-row` (44 px hit area, disabled = 38 % opacity, one pulse per session, one-time hint via localStorage `UNDO_HINT_KEY`; on phones (≤768 px) `.controls-below` becomes a 3-column grid (1fr / 128px / 1fr): row 1 = power (col 1, centred) · wheel (col 2) · undo/redo stacked vertically (col 3); row 2 = full-width sliders; row 3 = `.presets-area` as a column of centred `.preset-bank`s — `_renderPresetsContent(true)` splits saved colour presets, live colour/temperature swatches (left-aligned bank with a top divider) and effect/mode buttons into separate banks so dynamic content only grows downward and never moves the wheel, power, undo/redo, sliders or saved presets; without `.has-history` the wheel and power are centred as a pair; `.preset-bank` is `display: contents` on desktop and in floating mode, which keeps the earlier mirrored layout); `_updateHistoryButtons` syncs them; Ctrl/Cmd+Z and Ctrl+Y/Cmd+Shift+Z prefer light undo/redo outside edit mode. Config: `undo` (default false — opt-in), `undo_external` (default true, only matters with `undo`).
