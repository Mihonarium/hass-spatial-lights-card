class SpatialLightColorCard extends HTMLElement {
  ...
  setConfig(config) {
    ...
    this._config = {
      ...
      show_scene_create_button: !!config.show_scene_create_button,
  ...
  _renderAll() {
    ...
    this._els.sceneActionRow = this.shadowRoot.getElementById('sceneActionRow');
    ...
    return `
      * { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      .controls-floating {
        ...
        display: flex; gap: 20px; align-items: flex-start; justify-content: center; flex-wrap: wrap; box-shadow: var(--shadow-md);
      ...
      .controls-below {
        ...
        gap: 24px; align-items: flex-start; justify-content: center; flex-wrap: wrap;
      }
      ...
      .controls-stack { display:flex; flex-direction:column; gap:14px; min-width: 260px; flex:1; max-width: 540px; }
      .slider-group { display:flex; flex-direction:column; gap:12px; flex:1; }
      .slider-row { display:flex; align-items:center; gap:14px; padding: 10px 12px; border-radius: 14px; background: var(--surface-tertiary); box-shadow: var(--shadow-sm); }
      .slider-icon { font-size: 18px; opacity: 0.7; width: 22px; text-align:center; flex-shrink:0; }

      .slider {
        flex:1; -webkit-appearance:none; height:14px; border-radius:9999px; background: var(--surface-secondary);
        outline:none; position:relative; cursor:pointer; border:1px solid var(--border-medium); box-shadow: inset 0 1px 2px rgba(0,0,0,0.45);
      }
      .slider.temperature {
      ...
      .slider::-webkit-slider-thumb {
        -webkit-appearance:none; width:24px; height:24px; border-radius:9999px;
        background: var(--surface-primary); border:2px solid var(--text-primary); box-shadow: var(--shadow-sm);
        transition: transform var(--transition-fast), box-shadow var(--transition-fast);
      }
      .slider::-webkit-slider-thumb:hover { transform: scale(1.06); box-shadow: 0 2px 6px rgba(0,0,0,0.45); }
      .slider::-moz-range-thumb {
        width:24px; height:24px; border-radius:9999px; background: var(--surface-primary);
        border:2px solid var(--text-primary); box-shadow: var(--shadow-sm);
      }
      .slider-value { font-size: 13px; color: var(--text-secondary); min-width: 56px; text-align:right; font-weight: 700; }

      .action-row { display:flex; align-items:center; justify-content:flex-end; gap:10px; }
      .action-row.hidden { display:none; }
      .action-button {
        padding: 10px 14px; background: var(--surface-tertiary); border:1px solid var(--border-subtle);
        color: var(--text-primary); border-radius: 10px; cursor:pointer; font-size:13px; font-weight:700;
        transition: background var(--transition-fast), border-color var(--transition-fast), transform var(--transition-fast);
      }
      .action-button:hover { background: var(--surface-elevated); border-color: var(--border-medium); }
  ...
  _renderSceneActionRow() {
    const showButton = this._config.show_scene_create_button && this._selectedLights.size > 0;
    const label = showButton
      ? `Create scene from ${this._selectedLights.size} selected light${this._selectedLights.size === 1 ? '' : 's'}`
      : 'Select lights to create a scene';
    const hiddenClass = this._config.show_scene_create_button ? (showButton ? '' : 'hidden') : 'hidden';
    return `
      <div class="action-row ${hiddenClass}" id="sceneActionRow">
        <button class="action-button" id="createSceneBtn" ${showButton ? '' : 'disabled'}>${label}</button>
      </div>
    `;
  }
    ...
    return `
      <div class="settings-panel ${this._settingsOpen ? 'visible' : ''}" id="settingsPanel" role="dialog" aria-label="Settings">
        ...
        <div class="settings-section">
          ...
          <div class="settings-option">
            <span>Show Scene Creation</span>
            <button class="toggle ${this._config.show_scene_create_button ? 'on' : ''}" id="sceneToggle" role="switch" aria-checked="${this._config.show_scene_create_button}" aria-label="Show create scene button"></button>
          </div>
        </div>
        <div class="settings-section">
  ...
  _attachEventListeners() {
    ...
    if (this._els.sceneToggle) {
      this._els.sceneToggle.addEventListener('click', () => {
        ...
        this._els.sceneToggle.classList.toggle('on', this._config.show_scene_create_button);
        this._els.sceneToggle.setAttribute('aria-checked', String(this._config.show_scene_create_button));
        this._syncSceneActionVisibility();
      });
    }
    ...
    if (this._els.createSceneBtn) {
      this._els.createSceneBtn.addEventListener('click', () => this._createSceneFromSelection());
    }
  }
  ...
  _createSceneFromSelection() {
    if (!this._hass || !this._config.show_scene_create_button) return;
    const entities = [...this._selectedLights];
    if (entities.length === 0) return;

    const friendlyBase = entities.length === 1
      ? this._generateLabel(entities[0])
      : 'Selected lights scene';
    const defaultName = friendlyBase || 'New scene';
    const namePrompt = typeof window !== 'undefined' && typeof window.prompt === 'function'
      ? window.prompt('Name your new scene (how it will appear in Home Assistant):', defaultName)
      : defaultName;
    if (namePrompt == null) return;
    const sceneName = String(namePrompt).trim();
    if (!sceneName) return;

    const defaultId = this._slugifySceneId(sceneName) || this._slugifySceneId(`${friendlyBase}_scene`) || 'new_scene';
    const idPrompt = typeof window !== 'undefined' && typeof window.prompt === 'function'
      ? window.prompt('Scene ID (used for the entity_id, lowercase letters/numbers/underscores):', defaultId)
      : defaultId;
    if (idPrompt == null) return;
    const sceneId = this._slugifySceneId(idPrompt);
    if (!sceneId) return;
  ...
  _syncSceneActionVisibility() {
    if (!this._els.sceneActionRow || !this._els.createSceneBtn) return;
    const showButton = this._config.show_scene_create_button && this._selectedLights.size > 0;
    this._els.sceneActionRow.classList.toggle('hidden', !showButton);
    this._els.createSceneBtn.disabled = !showButton;
    const label = showButton
      ? `Create scene from ${this._selectedLights.size} selected light${this._selectedLights.size === 1 ? '' : 's'}`
      : 'Select lights to create a scene';
    this._els.createSceneBtn.textContent = label;
  }
  ...
  static getStubConfig() {
    return {
      ...
      show_scene_create_button: false,
    };
  }
}
