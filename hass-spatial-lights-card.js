class SpatialLightColorCard extends HTMLElement {
    ...
    return `
      * { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      .controls-below {
        ...
        gap: 24px; align-items: stretch; justify-content: center;
      }
      ...
      .controls-stack { display:flex; flex-direction:column; gap:14px; flex:1; min-width: 260px; max-width: 520px; }
      .slider-group { display:flex; flex-direction:column; gap:14px; width: 100%; }
      .slider-row { display:flex; align-items:center; gap:12px; padding: 10px 14px; border-radius: 16px; background: var(--surface-tertiary); border: 1px solid var(--border-subtle); box-shadow: var(--shadow-sm); }
      .slider-icon { font-size: 18px; opacity: 0.8; width: 22px; text-align:center; flex-shrink:0; }

      .slider {
        flex:1; -webkit-appearance:none; height:12px; border-radius:9999px; background: linear-gradient(to right, rgba(255,255,255,0.16), rgba(255,255,255,0.08));
      ...
      .slider::-moz-range-thumb {
        width:24px; height:24px; border-radius:9999px; background: var(--text-primary);
      ...
      .slider-value { font-size: 14px; color: var(--text-secondary); min-width: 60px; text-align:right; font-weight: 700; letter-spacing: 0.01em; }

      .action-row { display:flex; align-items:center; justify-content:flex-end; gap:10px; margin-top: 4px; width: 100%; }
      .action-row.hidden { display:none; }
      .action-button {
        padding: 10px 14px; background: var(--surface-elevated); border:1px solid var(--border-medium);
        color: var(--text-primary); border-radius: 10px; cursor:pointer; font-size:13px; font-weight:700;
        transition: background var(--transition-fast), border-color var(--transition-fast), transform var(--transition-fast), box-shadow var(--transition-fast);
        box-shadow: var(--shadow-sm);
      }
      .action-button:hover { background: var(--surface-primary); border-color: var(--accent-primary); box-shadow: var(--shadow-md); }
  ...
  _createSceneFromSelection() {
    ...
    const friendlyBase = entities.length === 1
      ? (this._hass.states[entities[0]]?.attributes?.friendly_name || this._generateLabel(entities[0]))
      : `Selected lights (${entities.length})`;
    const defaultName = `${friendlyBase} scene`;
    const nameInput = typeof window !== 'undefined' && typeof window.prompt === 'function'
      ? window.prompt('Name for the new scene:', defaultName)
      : defaultName;
    const sceneName = (nameInput || defaultName).trim();
    if (!sceneName) return;

    const defaultId = this._slugifySceneId(sceneName) || 'new_scene';
    const idInput = typeof window !== 'undefined' && typeof window.prompt === 'function'
      ? window.prompt('Enter a scene ID (used internally) to save this snapshot:', defaultId)
      : defaultId;
    const sceneId = this._slugifySceneId(idInput || '');
    if (!sceneId) return;
