class SpatialLightColorCard extends HTMLElement {
    ...
    return `
      * { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      .slider {
        ...
        --slider-track-active: linear-gradient(to right,
          var(--slider-fill) 0%,
          var(--slider-fill) calc(var(--slider-progress) * 100%),
          var(--surface-tertiary) calc(var(--slider-progress) * 100%),
          var(--surface-tertiary) 100%);
        height: var(--slider-height);
        border-radius: var(--slider-track-radius);
        background:
          var(--slider-track-active),
          linear-gradient(to right, var(--surface-tertiary) 0%, var(--surface-tertiary) 100%);
        background-size:
          calc((100% - var(--slider-thumb-size)) * var(--slider-progress) + (var(--slider-thumb-size) / 2)) 100%,
          100% 100%;
        background-repeat: no-repeat, no-repeat;
        background-position: left 0 center, left 0 center;
      ...
      .slider.temperature {
        ...
        background:
          linear-gradient(to right,
            ...
            rgba(255,255,255,0.18) calc(var(--slider-progress) * 100%),
            rgba(255,255,255,0.08) calc(var(--slider-progress) * 100%),
  ...
  _renderControlsFloating(visible, controlContext) {
    ...
    const tempPercent = (tempRange.max > tempRange.min)
      ...
      : 0;
    const brightnessProgress = brightnessPercent / 100;
    const tempProgress = tempPercent / 100;
      ...
      <div class="controls-floating ${visible ? 'visible' : ''}" id="controlsFloating" role="region" aria-label="Light controls" aria-live="polite">
        ...
        <div class="slider-group">
          <div class="slider-row">
            <input type="range" class="slider" id="brightnessSlider" min="0" max="255" value="${avgState.brightness}" aria-label="Brightness" style="--slider-progress:${brightnessProgress};--slider-fill:${brightnessColor};">
            <span class="slider-value" id="brightnessValue">${Math.round((avgState.brightness/255)*100)}%</span>
          </div>
          <div class="slider-row">
            <input type="range" class="slider temperature" id="temperatureSlider" min="${tempRange.min}" max="${tempRange.max}" value="${clampedTemp}" aria-label="Color temperature" style="--slider-progress:${tempProgress}">
  ...
  _updateControlValues(controlContext) {
    ...
    if (this._els.brightnessSlider) {
      ...
      this._els.brightnessSlider.style.setProperty('--slider-progress', `${brightnessProgress}`);
    ...
    if (this._els.temperatureSlider) {
      ...
      this._els.temperatureSlider.style.setProperty('--slider-progress', `${tempProgress}`);
    }
  ...
  _updateSliderVisual(el) {
    ...
    const percent = Number.isFinite(min) && Number.isFinite(max) && max > min
      ...
      : 0;
    el.style.setProperty('--slider-progress', `${percent / 100}`);
  }
