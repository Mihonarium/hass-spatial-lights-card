class SpatialLightColorCard extends HTMLElement {
  ...
  _renderAll() {
    ...
    this._els.colorWheel = this.shadowRoot.getElementById('colorWheel') || this.shadowRoot.getElementById('colorWheelMini');
    ...
    return `
      * { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      .controls-floating {
        ...
        display: flex; flex-direction: column; gap: 12px; align-items: stretch; box-shadow: var(--shadow-md);
        opacity: 0; pointer-events: none; transition: opacity var(--transition-base);
        z-index: 50; min-width: 420px;
      }
      .controls-floating.visible { opacity: 1; pointer-events: auto; }

      .controls-main { display:flex; gap: 20px; align-items: center; justify-content: center; }
      ...
      .controls-below {
        ...
        gap: 12px; align-items: stretch; justify-content: center; flex-direction: column;
      }
      ...
      .color-wheel {
        width: 260px; height: 260px; border-radius: 9999px; cursor: pointer;
        border: 2px solid var(--border-subtle); box-shadow: var(--shadow-sm); flex-shrink: 0;
      }

      .slider-group { display:flex; flex-direction:column; gap:14px; min-width: 240px; flex:1; max-width: 520px; }
      .slider-row { display:flex; align-items:center; gap:14px; min-height: 54px; padding: 8px 0; }
      .slider-icon { font-size: 17px; opacity: 0.72; width: 22px; text-align:center; flex-shrink:0; }

      .slider {
        flex:1; -webkit-appearance:none; height:12px; border-radius:9999px; background: var(--surface-tertiary);
      ...
      .action-row { display:flex; align-items:center; justify-content:flex-end; gap:10px; margin-top: 4px; padding-top: 8px; border-top: 1px solid var(--border-subtle); width: 100%; }
      .action-row.hidden { display:none; }
      .action-button {
        padding: 10px 14px; background: var(--surface-tertiary); border:1px solid var(--border-subtle);
        color: var(--text-primary); border-radius: 10px; cursor:pointer; font-size:14px; font-weight:600;
      ...
      @media (max-width: 768px) {
        ...
        .controls-floating { left: 16px; right: 16px; width: auto; transform: none; min-width: auto; }
  ...
  _renderControlsBelow(controlContext) {
    ...
    return `
      <div class="controls-below" id="controlsBelow" role="region" aria-label="Light controls" aria-live="polite">
        <div class="controls-main">
          <canvas id="colorWheel" class="color-wheel" width="360" height="360" role="img" aria-label="Color picker"></canvas>
          <div class="slider-group">
            <div class="slider-row">
              <span class="slider-icon" aria-hidden="true">💡</span>
              <input type="range" class="slider" id="brightnessSlider" min="0" max="255" value="${avgState.brightness}" aria-label="Brightness">
  ...
  _createSceneFromSelection() {
    ...
    const baseName = entities.length === 1
    ...
    const nameInput = typeof window !== 'undefined' && typeof window.prompt === 'function'
      ? window.prompt('Name your new scene:', defaultName)
      : defaultName;
    const sceneName = (nameInput || '').trim();
    if (!sceneName) return;

    const sceneId = this._slugifySceneId(sceneName) || this._slugifySceneId(baseName) || 'new_scene';
