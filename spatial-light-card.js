class SpatialLightColorCard extends HTMLElement {
  ...
  _handleTapActions(entity, shouldRegisterTap, pointerType = 'touch') {
    ...
    if (this._lastTap.entity === entity && (now - this._lastTap.time) < 350) {
  ...
  _attachEventListeners() {
    ...
    if (this._els.canvas) {
      ...
      this._els.canvas.addEventListener('dblclick', (e) => {
        const light = e.target.closest('.light');
        if (!light) return;
        e.preventDefault();
        this._toggleLight(light.dataset.entity);
        this._lastTap = { entity: null, time: 0 };
      });
    }
  ...
  _onPointerUp(e) {
    ...
    if (this._pendingTap && e.pointerId === this._pendingTap.pointerId) {
      const { entity, additive } = this._pendingTap;
      if (!this._longPressTriggered) {
        const newSelection = additive
          ? new Set(this._selectedLights)
          : new Set();
        if (additive && newSelection.has(entity)) newSelection.delete(entity);
        else newSelection.add(entity);
        this._commitSelection(newSelection);
      }
      this._handleTapActions(entity, !this._longPressTriggered, e.pointerType);
      this._pendingTap = null;
    }
  ...
  drawColorWheel() {
    ...
    if (!cssWidth || !cssHeight) {
      cssWidth = canvas.offsetWidth || canvas.clientWidth;
      cssHeight = canvas.offsetHeight || canvas.clientHeight;
    }
    if (!cssWidth || !cssHeight) {
      const fallback = parseInt(canvas.getAttribute('width'), 10) || 256;
      cssWidth = fallback;
      cssHeight = fallback;
      needsRetry = true;
    }
