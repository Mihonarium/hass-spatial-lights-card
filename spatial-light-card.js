class SpatialLightColorCard extends HTMLElement {
  constructor() {
    ...
    this._lastTap = null;
    this._suppressNextDblClick = false;
    this._suppressDblClickTimer = null;
  ...
  disconnectedCallback() {
    ...
    if (this._colorWheelFrame) {
      cancelAnimationFrame(this._colorWheelFrame);
      this._colorWheelFrame = null;
    }
    ...
    if (this._suppressDblClickTimer) {
      clearTimeout(this._suppressDblClickTimer);
      this._suppressDblClickTimer = null;
    }
    this._pendingTap = null;
    this._longPressTriggered = false;
  ...
  _attachEventListeners() {
    ...
    if (this._els.canvas) {
      ...
      this._els.canvas.addEventListener('dblclick', (e) => this._handleDoubleClick(e));
    }
  ...
  _handleDoubleClick(e) {
    ...
    if (!entity) return;
    e.preventDefault();
    this._toggleLightPower(entity);
  }

  _toggleLightPower(entity) {
  ...
  _onPointerDown(e) {
    ...
    if (targetLight) {
      ...
      if (isTouchLike) {
        if (this._lastTap && this._lastTap.entity === entity && (now - this._lastTap.time) < 350) {
          if (this._longPressTimer) {
            clearTimeout(this._longPressTimer);
            this._longPressTimer = null;
          }
          ...
          this._suppressNextDblClick = true;
          this._suppressDblClickTimer = setTimeout(() => {
            this._suppressNextDblClick = false;
            this._suppressDblClickTimer = null;
          }, 400);
          this._toggleLightPower(entity);
      ...
      if (this._lockPositions) {
        ...
        this._longPressTimer = setTimeout(() => {
          ...
          if (navigator.vibrate && (pointerType === 'touch' || pointerType === 'pen')) {
  ...
  drawColorWheel() {
    ...
    if (!cssWidth || !cssHeight) {
      ...
      if (style) {
        ...
        const sh = parseFloat(style.height);
        if (!Number.isNaN(sw)) cssWidth = sw;
        if (!Number.isNaN(sh)) cssHeight = sh;
      }
    }
    if (!cssWidth || !cssHeight) {
    ...
    if (!cssWidth || !cssHeight) {
      this._colorWheelFrame = requestAnimationFrame(() => {
        this._colorWheelFrame = null;
        this.drawColorWheel();
      });
      return;
