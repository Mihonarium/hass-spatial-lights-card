class SpatialLightColorCard extends HTMLElement {
  constructor() {
    ...
    this._lastTap = null; // { entity, time }
    this._lastTapTimeout = null;
  ...
  _observeColorWheel() {
    ...
    if (!this._els.colorWheel) return;

    const redraw = () => {
      if (!this._els.colorWheel) return;
      const rect = this._els.colorWheel.getBoundingClientRect();
    ...
    if (typeof ResizeObserver !== 'undefined') {
      ...
      this._resizeObserver.observe(this._els.colorWheel);
    }
  ...
  disconnectedCallback() {
    ...
    if (this._lastTapTimeout) {
      clearTimeout(this._lastTapTimeout);
      this._lastTapTimeout = null;
    }
    if (this._resizeObserver) {
      this._resizeObserver.disconnect();
  ...
  _attachEventListeners() {
    ...
    this.shadowRoot.querySelectorAll('.light').forEach(light => {
      light.addEventListener('dblclick', (ev) => {
        ev.preventDefault();
        ev.stopPropagation();
        const entity = light.dataset.entity;
        if (entity) {
          ...
          this._toggleEntityPower(entity);
        }
      });
    });
  ...
  _onPointerDown(e) {
    ...
    if (targetLight) {
      ...
      if (this._lockPositions) {
        ...
        this._scheduleLongPress(entity, pointerType);
    ...
    if (this._pendingTap && e.pointerId === this._pendingTap.pointerId) {
      if (!this._longPressTriggered) {
        ...
        if (tap.pointerType === 'touch' || tap.pointerType === 'pen') {
          if (this._lastTap && this._lastTap.entity === tap.entity && (now - this._lastTap.time) <= this._doubleTapDelay) {
            this._toggleEntityPower(tap.entity);
            this._clearLastTap();
            const newSelection = tap.additive
              ? new Set(this._selectedLights)
              : new Set();
            newSelection.add(tap.entity);
            this._commitSelection(newSelection);
          } else {
            this._registerTap(tap.entity, now);
            const newSelection = tap.additive
              ? new Set(this._selectedLights)
              : new Set();
            newSelection.add(tap.entity);
  ...
  _scheduleLongPress(entity, pointerType = 'mouse') {
    if (this._longPressTimer) {
      clearTimeout(this._longPressTimer);
      this._longPressTimer = null;
    }
    this._longPressTriggered = false;
    const delay = pointerType === 'mouse' ? Math.max(600, this._longPressDelay) : this._longPressDelay;
    this._longPressTimer = setTimeout(() => {
      this._longPressTimer = null;
      this._longPressTriggered = true;
      if (typeof navigator !== 'undefined' && navigator.vibrate && pointerType !== 'mouse') navigator.vibrate(30);
  ...
  _registerTap(entity, time) {
    this._lastTap = { entity, time };
    if (this._lastTapTimeout) clearTimeout(this._lastTapTimeout);
    this._lastTapTimeout = setTimeout(() => {
      this._clearLastTap();
  ...
  _toggleEntityPower(entity_id) {
    if (!this._hass) return;
    const st = this._hass.states[entity_id];
    if (!st) return;
    this._hass.callService('light', 'toggle', { entity_id });
  }
