class SpatialLightColorCard extends HTMLElement {
  constructor() {
    ...
    this._config = {};
    this._hass = null;
    this._temperatureBounds = { min: 2000, max: 6500 };
    ...
    /** Icon retries */
    this._iconRetryHandles = [];
    this._iconRetryActive = false;
    this._iconRetryCooldownUntil = 0;
  ...
  setConfig(config) {
    ...
    const parseKelvin = (value) => {
      if (value == null) return null;
      const num = Number(value);
      return Number.isFinite(num) ? num : null;
    };
    const parsedRange = Array.isArray(config.temperature_range) && config.temperature_range.length === 2
      ? [parseKelvin(config.temperature_range[0]), parseKelvin(config.temperature_range[1])]
      : null;
    if (parsedRange && parsedRange[0] != null && parsedRange[1] != null && parsedRange[0] > parsedRange[1]) {
      parsedRange.reverse();
    }
    ...
    this._config = {
      ...
      temperature_range: parsedRange && parsedRange.every(v => v != null) ? parsedRange : null,
      temperature_min: parsedRange && parsedRange[0] != null
        ? parsedRange[0]
        : parseKelvin(config.temperature_min),
      temperature_max: parsedRange && parsedRange[1] != null
        ? parsedRange[1]
        : parseKelvin(config.temperature_max),
    };
  ...
  set hass(hass) {
    ...
    this._hass = hass;
    this._temperatureBounds = this._computeTemperatureBounds();
    if (firstTime) {
      this._renderAll();
    } else {
      this.updateLights();
      if (this._config.show_entity_icons) {
        this._refreshIconsWithRetry();
      }
    }
  }
  ...
  _computeTemperatureBounds() {
    const clampKelvin = (value) => {
      if (value == null) return null;
      const num = Number(value);
      if (!Number.isFinite(num)) return null;
      return Math.min(10000, Math.max(1000, Math.round(num)));
    };

    let minK = clampKelvin(this._config.temperature_min);
    let maxK = clampKelvin(this._config.temperature_max);

    if (this._hass && (minK == null || maxK == null)) {
      this._config.entities.forEach(entity_id => {
        const st = this._hass?.states?.[entity_id];
        if (!st) return;
        const { min_mireds, max_mireds } = st.attributes || {};
        if (!min_mireds || !max_mireds) return;
        const entityWarm = Math.round(1000000 / max_mireds);
        const entityCool = Math.round(1000000 / min_mireds);
        if (Number.isFinite(entityWarm)) {
          const clampedWarm = clampKelvin(entityWarm);
          if (clampedWarm != null) {
            minK = minK == null ? clampedWarm : Math.min(minK, clampedWarm);
          }
        }
        if (Number.isFinite(entityCool)) {
          const clampedCool = clampKelvin(entityCool);
          if (clampedCool != null) {
            maxK = maxK == null ? clampedCool : Math.max(maxK, clampedCool);
          }
        }
      });
    }

    if (minK == null) minK = this._temperatureBounds?.min ?? 2000;
    if (maxK == null) maxK = this._temperatureBounds?.max ?? 6500;
    if (minK > maxK) {
      const tmp = minK;
      minK = maxK;
      maxK = tmp;
    ...
    const finalMin = clampKelvin(minK) ?? 2000;
    const finalMax = clampKelvin(maxK) ?? 6500;
    if (this._config.temperature_range) {
      this._config.temperature_range = [finalMin, finalMax];
    } else {
      if (this._config.temperature_min != null) this._config.temperature_min = finalMin;
      if (this._config.temperature_max != null) this._config.temperature_max = finalMax;
    }
    return { min: finalMin, max: finalMax };
  }

  _clampTemperature(value, bounds = null) {
    const activeBounds = bounds || this._temperatureBounds || { min: 2000, max: 6500 };
    const fallback = Math.round((activeBounds.min + activeBounds.max) / 2);
    if (!Number.isFinite(value)) return fallback;
    return Math.round(Math.min(activeBounds.max, Math.max(activeBounds.min, value)));
  }

  _ensureIconsVisible() {
    if (!this._config.show_entity_icons || !this.shadowRoot) return;
    const nodes = this.shadowRoot.querySelectorAll('.light');
    nodes.forEach(light => {
      const hasIcon = light.querySelector('.light-icon, ha-icon, ha-svg-icon, .light-icon-emoji');
      if (!hasIcon) {
  ...
  _refreshIconsWithRetry() {
    if (!this._config.show_entity_icons) return;
    this._ensureIconsVisible();
    const now = Date.now();
    if (this._iconRetryActive || this._iconRetryHandles.length > 0 || now < this._iconRetryCooldownUntil) return;
    this._iconRetryCooldownUntil = now + 2000;
    this._iconRetryActive = true;
    const delays = [120, 500, 1500];
    delays.forEach(delay => {
      const handle = setTimeout(() => {
        this._ensureIconsVisible();
        this._iconRetryHandles = this._iconRetryHandles.filter(id => id !== handle);
        if (this._iconRetryHandles.length === 0) {
          this._iconRetryActive = false;
        }
      }, delay);
      this._iconRetryHandles.push(handle);
    });
  }

  _clearIconRetryHandles() {
    if (this._iconRetryHandles.length) {
      this._iconRetryHandles.forEach(handle => clearTimeout(handle));
      this._iconRetryHandles = [];
    }
    this._iconRetryActive = false;
    this._iconRetryCooldownUntil = 0;
  ...
  _rearrangeAllLights() {
    ...
    const nextPositions = { ...this._config.positions };
    let changed = false;
    ...
    entities.forEach((entity, idx) => {
      ...
      const prev = this._config.positions[entity];
      nextPositions[entity] = pos;
    ...
    this._saveHistory();
    this._config.positions = nextPositions;
  ...
  _getAverageState() {
    ...
    const bounds = this._temperatureBounds || this._computeTemperatureBounds();
    const averagedTemp = tCnt ? Math.round(tTot / tCnt) : 4000;
    return {
      brightness: bCnt ? Math.round(bTot / bCnt) : 128,
      temperature: this._clampTemperature(averagedTemp, bounds),
      color: lastRGB,
    };
  }
  ...
  _renderAll() {
    ...
    const bounds = this._temperatureBounds || this._computeTemperatureBounds();
    const normalizedAvgState = {
      ...avgState,
      temperature: this._clampTemperature(avgState.temperature, bounds),
    };
      ...
      <ha-card>
        ...
        <div class="canvas-wrapper">
          <div class="canvas" id="canvas" role="application" aria-label="Spatial light control area">
            ...
            ${controlsPosition === 'floating' ? this._renderControlsFloating(showControls, normalizedAvgState, bounds) : ''}
            ${this._renderSettings()}
          </div>
          ${controlsPosition === 'below' ? this._renderControlsBelow(normalizedAvgState, bounds) : ''}
        </div>
    ...
    if ((showControls || this._config.always_show_controls) && this._els.colorWheel) {
      this.drawColorWheel();
      this._updateControlValues(normalizedAvgState);
    }
    this.updateLights();
    if (this._config.show_entity_icons) {
      this._refreshIconsWithRetry();
    } else {
      this._clearIconRetryHandles();
    }
  }
  ...
  _renderControlsFloating(visible, avgState, bounds) {
    return `
      <div class="controls-floating ${visible ? 'visible' : ''}" id="controlsFloating" role="region" aria-label="Light controls" aria-live="polite">
        ...
        <div class="slider-group">
          ...
          <div class="slider-row">
            ...
            <input type="range" class="slider temperature" id="temperatureSlider" min="${bounds.min}" max="${bounds.max}" value="${avgState.temperature}" aria-label="Color temperature">
  ...
  _renderControlsBelow(avgState, bounds) {
    return `
  ...
  _updateControlValues(avgState) {
    ...
    const bounds = this._temperatureBounds || this._computeTemperatureBounds();
    const clampedTemp = this._clampTemperature(avgState.temperature, bounds);
    if (this._els.temperatureSlider) {
      this._els.temperatureSlider.min = String(bounds.min);
      this._els.temperatureSlider.max = String(bounds.max);
      this._els.temperatureSlider.value = String(clampedTemp);
    }
    if (this._els.temperatureValue) {
      this._els.temperatureValue.textContent = `${clampedTemp}K`;
    }
  }
  ...
  _attachEventListeners() {
    ...
    if (this._els.iconToggle) {
      this._els.iconToggle.addEventListener('click', () => {
        ...
        if (this._config.show_entity_icons) this._refreshIconsWithRetry();
        else this._clearIconRetryHandles();
      });
    }
  ...
  _handleTemperatureInput(e) {
    ...
    const clamped = this._clampTemperature(inputVal, bounds);
    if (this._els.temperatureSlider) {
  ...
  _handleTemperatureChange() {
    ...
    const bounds = this._temperatureBounds || this._computeTemperatureBounds();
    const kelvin = this._clampTemperature(this._pendingTemperature, bounds);
  ...
  updateLights() {
    ...
    lights.forEach(light => {
      ...
      if (this._config.show_entity_icons) {
        const iconNode = light.querySelector('.light-icon, ha-icon, ha-svg-icon, .light-icon-emoji');
        if (!iconNode) {
          const iconData = this._getEntityIconData(id);
          light.insertAdjacentHTML('afterbegin', this._renderIcon(iconData));
        }
      }
    });
  ...
  _generateYAML() {
    ...
    if (this._config.temperature_range) {
      ...
      yaml += `${indent}- ${Math.round(this._config.temperature_range[0])}\n`;
      yaml += `${indent}- ${Math.round(this._config.temperature_range[1])}\n`;
    } else {
      if (this._config.temperature_min != null) yaml += `temperature_min: ${Math.round(this._config.temperature_min)}\n`;
      if (this._config.temperature_max != null) yaml += `temperature_max: ${Math.round(this._config.temperature_max)}\n`;
    }
