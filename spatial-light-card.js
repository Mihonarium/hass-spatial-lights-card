class SpatialLightColorCard extends HTMLElement {
  ...
  setConfig(config) {
    ...
    const explicitTempRange = config.temperature_range || {};
    const parseKelvin = (value) => {
      if (value == null) return null;
      const num = Number(value);
      return Number.isFinite(num) ? num : null;
    };
    const tempMin = parseKelvin(config.temperature_min ?? explicitTempRange.min);
    const tempMax = parseKelvin(config.temperature_max ?? explicitTempRange.max);
    ...
    const fallbackRange = {
      min: tempMin != null ? tempMin : 2000,
      max: tempMax != null ? tempMax : 6500,
    };
    if (fallbackRange.min > fallbackRange.max) {
      [fallbackRange.min, fallbackRange.max] = [fallbackRange.max, fallbackRange.min];
    }
    fallbackRange.min = Math.round(fallbackRange.min);
    fallbackRange.max = Math.round(fallbackRange.max);
    this._temperatureRange = fallbackRange;
  }
  ...
  _renderIcon(iconData) {
    if (iconData.type === 'mdi') {
      const icon = this._escapeAttribute(iconData.value);
      return `<ha-icon class="light-icon light-icon-mdi" data-icon="${icon}" icon="${icon}"></ha-icon>`;
    }
    if (iconData.type === 'emoji') {
      const emoji = this._escapeAttribute(iconData.value);
      return `<div class="light-icon light-icon-emoji">${emoji}</div>`;
    }
  ...
  _rearrangeAllLights() {
    ...
    const before = JSON.parse(JSON.stringify(this._config.positions));
    ...
    this._saveHistory(before);
    this._saveHistory(this._config.positions);
  ...
  /** ---------- History ---------- */
  _saveHistory(positions = this._config.positions) {
  ...
  _calculateTemperatureRange() {
    const fallback = { min: 2000, max: 6500 };
    const configuredMin = this._config?.temperature_min;
    const configuredMax = this._config?.temperature_max;

    let minKelvin = configuredMin != null ? configuredMin : null;
    let maxKelvin = configuredMax != null ? configuredMax : null;

    if (!this._hass || (!this._config?.entities?.length && minKelvin != null && maxKelvin != null)) {
      const min = minKelvin != null ? minKelvin : fallback.min;
      const max = maxKelvin != null ? maxKelvin : fallback.max;
      return min <= max ? { min, max } : { min: max, max: min };
    }

    let warmest = Infinity; // lowest Kelvin
    let coolest = 0;        // highest Kelvin

    (this._config?.entities || []).forEach(entity_id => {
      const st = this._hass?.states?.[entity_id];
      if (!st) return;
      const { min_mireds, max_mireds } = st.attributes || {};
      if (typeof max_mireds === 'number' && max_mireds > 0) {
        const warmKelvin = Math.round(1000000 / max_mireds);
        if (warmKelvin < warmest) warmest = warmKelvin;
      }
      if (typeof min_mireds === 'number' && min_mireds > 0) {
        const coolKelvin = Math.round(1000000 / min_mireds);
        if (coolKelvin > coolest) coolest = coolKelvin;
      }
    });

    if (!Number.isFinite(warmest) || warmest <= 0) warmest = minKelvin != null ? minKelvin : fallback.min;
    if (!Number.isFinite(coolest) || coolest <= 0) coolest = maxKelvin != null ? maxKelvin : fallback.max;

    if (minKelvin == null) minKelvin = warmest;
    if (maxKelvin == null) maxKelvin = coolest;

    if (minKelvin > maxKelvin) {
      const swapped = { min: maxKelvin, max: minKelvin };
      return swapped;
    }

    return {
      min: Math.round(Math.max(1000, minKelvin)),
      max: Math.round(Math.max(Math.max(1000, minKelvin), maxKelvin)),
    };
  }

  _applyTemperatureRangeToSlider() {
    if (!this._els.temperatureSlider || !this._temperatureRange) return;
    const { min, max } = this._temperatureRange;
    const slider = this._els.temperatureSlider;
    ...
    if (String(normalized) !== slider.value) {
      slider.value = String(normalized);
    }
    if (this._els.temperatureValue) {
      this._els.temperatureValue.textContent = `${normalized}K`;
    }
  }

  /** ---------- Aggregated state of selected lights ---------- */
  _getAverageState(range = this._temperatureRange || { min: 2000, max: 6500 }) {
    ...
    if (controlled.length === 0) {
      ...
      return { brightness: 128, temperature: midpoint, color: null };
    }
    ...
    return {
      ...
      temperature: (() => {
        const computed = tCnt ? Math.round(tTot / tCnt) : Math.round((range.min + range.max) / 2);
        return Math.min(range.max, Math.max(range.min, computed));
      })(),
      color: lastRGB,
    };
  }
  ...
  /** ---------- Rendering ---------- */
  _renderAll() {
    this._temperatureRange = this._calculateTemperatureRange();
    const avgState = this._getAverageState(this._temperatureRange);
      ...
      <ha-card>
        ...
        <div class="canvas-wrapper">
          <div class="canvas" id="canvas" role="application" aria-label="Spatial light control area">
            ...
            ${controlsPosition === 'floating' ? this._renderControlsFloating(showControls, avgState, this._temperatureRange) : ''}
            ${this._renderSettings()}
          </div>
          ${controlsPosition === 'below' ? this._renderControlsBelow(avgState, this._temperatureRange) : ''}
        </div>
    ...
    this._applyTemperatureRangeToSlider();
    this._attachEventListeners();
    this._hydrateIconNodes();
  ...
  _renderControlsFloating(visible, avgState, tempRange) {
    return `
      <div class="controls-floating ${visible ? 'visible' : ''}" id="controlsFloating" role="region" aria-label="Light controls" aria-live="polite">
        ...
        <div class="slider-group">
          ...
          <div class="slider-row">
            ...
            <input type="range" class="slider temperature" id="temperatureSlider" min="${tempRange.min}" max="${tempRange.max}" value="${avgState.temperature}" aria-label="Color temperature">
  ...
  _renderControlsBelow(avgState, tempRange) {
    return `
  ...
  _updateControlValues(avgState) {
    ...
    if (this._els.temperatureSlider) {
      ...
      const clamped = this._temperatureRange
        ? Math.min(this._temperatureRange.max, Math.max(this._temperatureRange.min, avgState.temperature))
  ...
  _hydrateIconNodes() {
    ...
    const nodes = Array.from(this.shadowRoot.querySelectorAll('ha-icon[data-icon]'));
    ...
    const apply = () => {
      nodes.forEach(node => {
        ...
        if (node.getAttribute('icon') !== icon) node.setAttribute('icon', icon);
      });
    };
    ...
    if (registry?.whenDefined) {
      registry.whenDefined('ha-icon').then(schedule);
    } else {
      schedule();
    }
  }
  ...
  _handleTemperatureInput(e) {
    ...
    const range = this._temperatureRange;
    const clamped = range ? Math.min(range.max, Math.max(range.min, k)) : k;
    if (clamped !== k) {
  ...
  updateLights() {
    ...
    const recalculatedRange = this._calculateTemperatureRange();
    if (recalculatedRange && (!this._temperatureRange || recalculatedRange.min !== this._temperatureRange.min || recalculatedRange.max !== this._temperatureRange.max)) {
      this._temperatureRange = recalculatedRange;
      this._applyTemperatureRangeToSlider();
    }
  ...
  _generateYAML() {
    ...
    if (this._config.temperature_min != null) yaml += `temperature_min: ${this._config.temperature_min}\n`;
    if (this._config.temperature_max != null) yaml += `temperature_max: ${this._config.temperature_max}\n`;
