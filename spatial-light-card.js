class SpatialLightColorCard extends HTMLElement {
  constructor() {
    ...
    this._preOverlaySelection = null;
  }
  ...
  _refreshEntityIcons(attempt = 0) {
    ...
    const ensureDefined = () => {
      ...
      if (unresolved) {
        if (attempt >= 2) {
          this._nudgeIconVisibility();
        }
        if (attempt >= 4) {
          this._forceIconRebuild();
        }
        if (attempt < 8) {
          this._scheduleIconRefresh(attempt + 1, 220 * (attempt + 1));
        }
      }
    };
  ...
  _nudgeIconVisibility() {
    if (!this.shadowRoot || !this._config.show_entity_icons) return;
    const icons = this.shadowRoot.querySelectorAll('.light > ha-icon, .light > .light-icon-emoji');
    if (!icons.length) return;
    icons.forEach(iconEl => {
  ...
  _forceIconRebuild() {
    ...
    lights.forEach(light => {
      ...
      if (!entity) return;
      const currentIcon = light.querySelector('.light-icon, ha-icon, ha-svg-icon, .light-icon-emoji');
    ...
    if (rebuilt) {
      this._scheduleIconRefresh(0, 80);
    }
  }

  _setGlobalOverlayState(active) {
    if (active) {
      if (this._overlayActive) return;
      this._overlayActive = true;
      this.setAttribute('data-global-overlay-active', 'true');
      if (!this._preOverlaySelection) {
        this._preOverlaySelection = new Set(this._selectedLights);
      }
      ...
      if (!this._overlayActive) return;
      this._overlayActive = false;
      this.removeAttribute('data-global-overlay-active');
      const restore = this._preOverlaySelection;
      this._preOverlaySelection = null;
      if (restore && restore.size > 0) {
        this._commitSelection(restore);
      }
    }
  }
  ...
  _openMoreInfo(entity) {
    this._setGlobalOverlayState(true);
  ...
  _styles() {
    return `
      * { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      ha-card { background: var(--surface-primary); overflow: hidden; font-family: var(--font-sans); position: relative; z-index: 0; }
      ...
      .settings-panel {
        ...
        transition: opacity var(--transition-base), transform var(--transition-base); z-index:14;
      }
      ...
      :host([data-global-overlay-active='true']) .light,
      :host([data-global-overlay-active='true']) .controls-floating,
      :host([data-global-overlay-active='true']) .settings-panel,
      :host([data-global-overlay-active='true']) .selection-box {
        z-index: 0 !important;
      }
      :host([data-global-overlay-active='true']) .canvas,
      :host([data-global-overlay-active='true']) .controls-floating,
      :host([data-global-overlay-active='true']) .settings-panel,
      :host([data-global-overlay-active='true']) .selection-box {
        pointer-events: none;
  ...
  connectedCallback() {
    ...
    if (typeof window !== 'undefined') {
      ...
      this._boundMoreInfo = (ev) => {
        const detail = ev?.detail || {};
        const entityId = detail.entityId ?? detail.entity_id ?? null;
        if (entityId) {
          this._setGlobalOverlayState(true);
        } else {
          this._setGlobalOverlayState(false);
        }
      };
      window.addEventListener('hass-more-info', this._boundMoreInfo);
    }
  }
