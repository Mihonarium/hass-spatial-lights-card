class SpatialLightColorCard extends HTMLElement {
  constructor() {
    ...
    this._temperatureRange = { min: 2000, max: 6500 };
    this._explicitTemperatureRange = null;
  ...
  setConfig(config) {
    ...
    this._explicitTemperatureRange = this._normalizeTemperatureConfig(config);
    this._normalizeConfiguredPositions();
    this._gridSize = this._config.grid_size;
    this._applyTemperatureRange(this._resolveTemperatureRange());
  ...
  set hass(hass) {
    ...
    this._hass = hass;
    this._maybeUpdateTemperatureRange(true);
    if (firstTime) {
      this._renderAll();
    } else {
      this.updateLights();
  ...
  _initializePositions() {
    ...
    unpos.forEach((entity, idx) => {
      ...
      this._setEntityPosition(entity, x, y);
    });
  }
  ...
  _rearrangeAllLights() {
    ...
    entities.forEach((entity, idx) => {
      ...
      this._setEntityPosition(entity, pos.x, pos.y);
    });
  ...
  _smoothApplyPositions() {
    ...
    lights.forEach(light => {
      ...
      if (pos) {
        ...
        light.dataset.x = String(this._clampPercent(pos.x));
        light.dataset.y = String(this._clampPercent(pos.y));
  ...
  _setEntityPosition(entity, x, y) {
    if (!this._config.positions) this._config.positions = {};
    const clampedX = this._clampPercent(x);
    const clampedY = this._clampPercent(y);
    this._config.positions[entity] = { x: clampedX, y: clampedY };
  ...
  _normalizeConfiguredPositions() {
    if (!this._config.positions) return;
    Object.entries(this._config.positions).forEach(([entity, pos]) => {
      ...
      const x = parseValue(pos.x);
      const y = parseValue(pos.y);
      const safeX = x == null ? 50 : this._clampPercent(x);
      const safeY = y == null ? 50 : this._clampPercent(y);
      this._config.positions[entity] = { x: safeX, y: safeY };
    });
  }

  _normalizeTemperatureConfig(config) {
    const range = { min: null, max: null };
    if (Array.isArray(config.temperature_range) && config.temperature_range.length === 2) {
      range.min = Number(config.temperature_range[0]);
      range.max = Number(config.temperature_range[1]);
    }
    if (config.temperature_min != null) range.min = Number(config.temperature_min);
    if (config.temperature_max != null) range.max = Number(config.temperature_max);
    const isValid = (val) => Number.isFinite(val) && val > 0;
    return {
      min: isValid(range.min) ? Math.round(range.min) : null,
      max: isValid(range.max) ? Math.round(range.max) : null,
    };
  }

  _resolveTemperatureRange() {
    if (this._explicitTemperatureRange && (this._explicitTemperatureRange.min != null || this._explicitTemperatureRange.max != null)) {
      const fallback = { min: 2000, max: 6500 };
      const min = this._explicitTemperatureRange.min != null ? this._explicitTemperatureRange.min : fallback.min;
      const max = this._explicitTemperatureRange.max != null ? this._explicitTemperatureRange.max : fallback.max;
      return this._normalizeKelvinBounds(min, max);
    }
    const derived = this._deriveTemperatureRangeFromStates();
    if (derived) return derived;
    return { min: 2000, max: 6500 };
  }

  _normalizeKelvinBounds(min, max) {
    const safeMin = this._clampKelvin(min, true);
    const safeMax = this._clampKelvin(max, true);
    if (safeMin >= safeMax) {
      const midpoint = Math.max(1000, Math.min(9000, Math.round((safeMin + safeMax) / 2)) || 4000);
      return { min: Math.max(1000, midpoint - 500), max: Math.min(9000, midpoint + 500) };
    }
    return { min: safeMin, max: safeMax };
  }

  _clampKelvin(value, skipRange = false) {
    const num = Number(value);
    if (!Number.isFinite(num)) {
      return Math.round((this._temperatureRange.min + this._temperatureRange.max) / 2);
    }
    const lower = skipRange ? 1000 : this._temperatureRange.min;
    const upper = skipRange ? 9000 : this._temperatureRange.max;
    return Math.max(lower, Math.min(upper, Math.round(num)));
  }

  _deriveTemperatureRangeFromStates() {
    ...
    this._config.entities.forEach(entity => {
      ...
      let minK = attrs.min_color_temp_kelvin;
      let maxK = attrs.max_color_temp_kelvin;
      if ((!minK || !maxK) && Array.isArray(attrs.color_temp_range) && attrs.color_temp_range.length === 2) {
        const [miredMin, miredMax] = attrs.color_temp_range;
        minK = Math.round(1000000 / miredMax);
        maxK = Math.round(1000000 / miredMin);
      }
      if ((!minK || !maxK) && attrs.min_mireds && attrs.max_mireds) {
        minK = Math.round(1000000 / attrs.max_mireds);
        maxK = Math.round(1000000 / attrs.min_mireds);
      }
  ...
  _applyTemperatureRange(range) {
    if (!range) return;
    this._temperatureRange = {
      min: this._clampKelvin(range.min, true),
      max: this._clampKelvin(range.max, true),
    };
    if (this._temperatureRange.min > this._temperatureRange.max) {
      const mid = Math.round((this._temperatureRange.min + this._temperatureRange.max) / 2) || 4000;
      this._temperatureRange = { min: Math.max(1000, mid - 500), max: Math.min(9000, mid + 500) };
    }
    this._updateTemperatureSliderBounds();
  }

  _updateTemperatureSliderBounds() {
    if (this._els.temperatureSlider) {
      this._els.temperatureSlider.min = String(this._temperatureRange.min);
      this._els.temperatureSlider.max = String(this._temperatureRange.max);
      const gradient = this._buildTemperatureGradient();
      if (gradient) {
        this._els.temperatureSlider.style.background = gradient;
      }
    }
  }

  _buildTemperatureGradient() {
    const toColor = (kelvin) => {
      const k = Math.max(1000, Math.min(9000, kelvin));
      const t = k / 100;
      let r, g, b;
      if (t <= 66) {
        r = 255;
    ...
    const min = this._temperatureRange.min;
    const max = this._temperatureRange.max;
    if (!Number.isFinite(min) || !Number.isFinite(max)) return null;
    const mid = Math.round((min + max) / 2);
    return `linear-gradient(to right, ${toColor(min)} 0%, ${toColor(mid)} 50%, ${toColor(max)} 100%)`;
  }

  _maybeUpdateTemperatureRange(force = false) {
    const next = this._resolveTemperatureRange();
    if (!next) return;
    if (
      force ||
      next.min !== this._temperatureRange.min ||
      next.max !== this._temperatureRange.max
    ) {
      this._applyTemperatureRange(next);
    }
  }

  _scheduleIconRefresh() {
    if (this._iconRefreshTimer) {
      clearTimeout(this._iconRefreshTimer);
      this._iconRefreshTimer = null;
    }
    if (!this._config.show_entity_icons) return;
    if (typeof window === 'undefined') return;
    this._iconRefreshTimer = setTimeout(() => {
      this._iconRefreshTimer = null;
      this._bindIconElements(true);
    }, 250);
  }

  _bindIconElements(force = false) {
    if (!this._config.show_entity_icons) return;
    const ensureDefined = customElements?.whenDefined ? customElements.whenDefined('ha-icon') : Promise.resolve();
    ensureDefined.then(() => {
      const icons = this.shadowRoot?.querySelectorAll('.light ha-icon');
      if (!icons) return;
      icons.forEach(iconEl => {
        const target = iconEl;
        if (force || !target.icon) {
          const attr = target.getAttribute('icon');
          if (attr) target.icon = attr;
        }
        if ('hass' in target && this._hass) {
  ...
  _getAverageState() {
    ...
    if (controlled.length === 0) {
      const mid = Math.round((this._temperatureRange.min + this._temperatureRange.max) / 2);
      return { brightness: 128, temperature: mid, color: null };
    }
    ...
    const avgTemp = tCnt ? Math.round(tTot / tCnt) : Math.round((this._temperatureRange.min + this._temperatureRange.max) / 2);
    return {
      brightness: bCnt ? Math.round(bTot / bCnt) : 128,
      temperature: this._clampKelvin(avgTemp),
      color: lastRGB,
    };
  }
  ...
  _renderLightsHTML() {
    return this._config.entities.map(entity_id => {
      const stored = this._config.positions[entity_id];
      const pos = stored ? {
        x: this._clampPercent(Number(stored.x)),
        y: this._clampPercent(Number(stored.y)),
      } : { x: 50, y: 50 };
      const st = this._hass?.states[entity_id];
      if (!st) return '';
      ...
      <div class="controls-floating ${visible ? 'visible' : ''}" id="controlsFloating" role="region" aria-label="Light controls" aria-live="polite">
        ...
        <div class="slider-group">
          ...
          <div class="slider-row">
            ...
            <input type="range" class="slider temperature" id="temperatureSlider" min="${this._temperatureRange.min}" max="${this._temperatureRange.max}" value="${avgState.temperature}" aria-label="Color temperature">
  ...
  _updateControlValues(avgState) {
    ...
    if (this._els.temperatureSlider) {
      this._updateTemperatureSliderBounds();
      const clamped = this._clampKelvin(avgState.temperature);
      this._els.temperatureSlider.value = String(clamped);
    }
    if (this._els.temperatureValue) {
      const clamped = this._clampKelvin(avgState.temperature);
      this._els.temperatureValue.textContent = `${clamped}K`;
    }
  }
  ...
  _onPointerDown(e) {
    ...
    if (targetLight) {
      ...
      const fallbackPos = this._config.positions[entity] || { x: 50, y: 50 };
      const inlineX = parseFloat(targetLight.style.left);
      const inlineY = parseFloat(targetLight.style.top);
      this._dragState = {
        entity,
        startX: e.clientX,
        startY: e.clientY,
        initialLeft: Number.isFinite(inlineX) ? inlineX : this._clampPercent(fallbackPos.x),
        initialTop: Number.isFinite(inlineY) ? inlineY : this._clampPercent(fallbackPos.y),
        rect,
        moved: false,
      };
    ...
    if (this._dragState) {
      ...
      this._raf = requestAnimationFrame(() => {
        ...
        if (node) {
          ...
          node.dataset.x = String(xPercent);
          node.dataset.y = String(yPercent);
        }
      });
      return;
    }
    ...
    if (this._dragState) {
      ...
      if (node) {
        ...
        this._setEntityPosition(
          entity,
          Number.isFinite(finalLeft) ? finalLeft : this._clampPercent(node.dataset.x),
          Number.isFinite(finalTop) ? finalTop : this._clampPercent(node.dataset.y)
        );
      }
      if (moved) {
        this._saveHistory();
  ...
  _cancelActiveInteractions() {
    ...
    this._pendingTap = null;
    this._longPressTriggered = false;
    this.shadowRoot.querySelectorAll('.light.dragging').forEach(light => light.classList.remove('dragging'));
  }
  ...
  _handleTemperatureInput(e) {
    ...
    const clamped = this._clampKelvin(k);
    if (this._els.temperatureValue) this._els.temperatureValue.textContent = `${clamped}K`;
    this._pendingTemperature = clamped;
  }
  _handleTemperatureChange() {
    ...
    const clamped = this._clampKelvin(this._pendingTemperature);
    const mireds = Math.round(1000000 / clamped);
    if (this._els.temperatureSlider) {
      this._els.temperatureSlider.value = String(clamped);
    }
    if (this._els.temperatureValue) {
  ...
  updateLights() {
    ...
    this._maybeUpdateTemperatureRange();
    const lights = this.shadowRoot.querySelectorAll('.light');
    lights.forEach(light => {
  ...
  _generateYAML() {
    ...
    if (this._explicitTemperatureRange) {
      const { min, max } = this._explicitTemperatureRange;
      if (min != null && max != null) {
        yaml += `temperature_range:\n`;
        yaml += `${indent}- ${Math.round(min)}\n`;
        yaml += `${indent}- ${Math.round(max)}\n`;
      } else {
        if (min != null) yaml += `temperature_min: ${Math.round(min)}\n`;
        if (max != null) yaml += `temperature_max: ${Math.round(max)}\n`;
      }
    }
